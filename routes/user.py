...
async def user_signup(data: SignupRequest):
    ...
    where_clause = []
    if data.email: where_clause.append({"email": data.email})
    if data.phone: where_clause.append({"phone": data.phone})
    ...
    token = create_access_token(data={"sub": str(user.id), "role": "user"})
    return {"message": "User registered successfully", "access_token": token, "user": user}
...
async def forgot_password(data: ForgotPasswordRequest):
    ...
    await db.passwordreset.upsert(
        where={"token": token},
        data={
            "create": {"email": data.email, "token": token, "expiresAt": expires},
...
async def request_signup_otp(data: OTPRequest):
    ...
    # 3. Print to console (Mock sending)
